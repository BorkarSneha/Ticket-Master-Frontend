...
export const startAddEmployee=(formData,props)=>{
    return(dispatch)=>{
        axios.post('http://localhost:3025/employees',formData ,{
            headers: {'x-auth':localStorage.getItem('authToken')}
        })
        .then((response)=>{
...
export const startEditEmployee=(formData,props)=>{
     return (dispatch)=>{
        axios.put(`http://localhost:3025/employees/${props.match.params.id}`,formData ,{
        headers: {'x-auth':localStorage.getItem('authToken')}
    })
    .then((response)=>{
        if(response.data.hasOwnProperty('errors')){
...
export const startRemoveEmployee=(id)=>{
       return(dispatch)=>{
        axios.delete(`http://localhost:3025/employees/${id}`,{
            headers: {'x-auth':localStorage.getItem('authToken')}
        })
        .then((response)=>{
